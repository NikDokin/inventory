...
func (pg *Adapter) UpdateCommodityQuantity(ctx context.Context, tx types.Tx, commodityID string, quantity int) error {
	query := `
		UPDATE commodities
	...
	rowsAffected, err := tx.Exec(ctx, query, quantity, commodityID)
	if err != nil {
		return fmt.Errorf("failed to exec: %w", err)
	}
	if rowsAffected == 0 {
